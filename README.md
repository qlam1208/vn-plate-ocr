...
Chuỗi X được đưa vào mạng Bi-LSTM. Khác với CNN chỉ nhìn cục bộ, Bi-LSTM quét chuỗi theo 2 chiều (từ trái qua phải và ngược lại) để nắm bắt ngữ cảnh. Tại mỗi bước thời gian t, trạng thái ẩn h_t được tính bằng cách ghép trạng thái của cả 2 chiều (h_forward và h_backward):

    h_t = [ h_forward_t , h_backward_t ]

Đầu ra của phân lớp này là một ma trận xác suất Y, chứa phân bố xác suất của toàn bộ tập ký tự (cộng thêm ký tự `Blank`) tại mỗi cột t.

### 3. Căn chỉnh và Giải mã (CTC Loss)
...
Hàm ánh xạ (B) sẽ tự động gộp các ký tự giống nhau đứng liền kề và xóa bỏ `-`. 
Ví dụ: B( --22-99-C-11- ) = 29C1.

Công thức CTC Loss:
Mục tiêu của mô hình là cực đại hóa xác suất sinh ra chuỗi nhãn S đúng. Xác suất của S là tổng xác suất của mọi đường dẫn (paths) có thể ánh xạ ra S:

    P(S | X) = SUM( P(path | X) )

Hàm mất mát (Loss) cần tối thiểu hóa được định nghĩa là âm logarit của xác suất này:
...
- Bộ Lọc Regex Thông Minh: Tích hợp sẵn bộ luật Regex xử lý được hầu hết các định dạng biển số tại VN.
- Đánh Giá Tự Động: Script tự động chấm điểm Accuracy (Độ chính xác nguyên biển) và CER (Tỉ lệ lỗi ký tự) qua các epoch, vẽ biểu đồ và trích xuất model tốt nhất.
